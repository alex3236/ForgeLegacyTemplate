...
injection types like `@WrapOperation`, `@WrapWithCondition`, `@ModifyExpressionValue`, and more. See the examples in 
`MixinGuiMainMenuExtras.java` and `MixinExtrasExamples.java` for usage. If you don't want MixinExtras, remove the 
MixinExtras dependency lines from `build.gradle.kts`.

### MixinExtras Features

MixinExtras provides several advanced injection types:

- **@WrapOperation**: Wrap method calls to modify parameters or return values
- **@WrapWithCondition**: Conditionally prevent method calls from executing  
- **@ModifyExpressionValue**: Modify the return value of expressions and method calls
- **@ModifyReceiver**: Modify the receiver (this) of method calls
- **@ModifyReturnValue**: Modify the return value of methods

For more information, see the [MixinExtras documentation](https://github.com/LlamaLad7/MixinExtras/wiki).
